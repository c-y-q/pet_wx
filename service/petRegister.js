...
exports.queryRegList = async (openId, unionId) => {
  ...
  const resultSql = ` select p.expire_time,p.birthday,p.pay_type,p.gender,p.breed,p.coat_color, p.id,m.real_name,m.id_number,m.residential_address,m.contact_phone,s.name,p.dog_reg_num,p.pet_name,p.pet_state,p.renew_time,p.create_time,p.pet_photo_url 
...
exports.yearCheck = async (openId, petRegId, options, dogRegNum, orderNum) => {
  ...
  if (wxpetRegResult.length == 0) {
    ...
    const copyToWxPetMasterPromise = conn.query(
      `insert into wx_pet_master(year,pet_reg_id,photo_url,creator_id,create_time,update_time,photo_url2) select year,pet_reg_id,photo_url,creator_id,create_time,update_time,photo_url2 from pet_master where id = '${petRegInfo[0].master_id}' `
    );
...
exports.updatePetRegInfo = async options => {
  ...
  const addRecordSql =
    "update  wx_addpet_record set audit_status = 0   where pet_id =? ";
...
exports.updateYearCheckInfo = async (petRegId, options) => {
  ...
  //更改年审记录
  const updateSql2 =
    "update  wx_review_record set  audit_status = 0, update_time = ?  where pet_id = ? and order_num = ? ";
  await await conn.query(updateSql2, [
    moment().format("YYYYMMDDHHmmss"),
    petRegId,
    options.orderNum
  ]);
...
exports.addinformations = async (params, petRegId, uuid, orderNum) => {
  ...
  const handleParams = {
    ...
    petPhotoUrl:
      (params.petPhotoUrl && params.petPhotoUrl.replace(imgHttp, imgDbPath)) ||
      "",
    ...
    idNumberPic1:
      (params.idNumberPic1 &&
        params.idNumberPic1.replace(imgHttp, imgDbPath)) ||
      "",
    idNumberPic2:
      (params.idNumberPic2 &&
        params.idNumberPic2.replace(imgHttp, imgDbPath)) ||
      "",
    residencePermitPic:
      (params.residencePermitPic &&
        params.residencePermitPic.replace(imgHttp, imgDbPath)) ||
      "",
    residencePermitPic2:
      (params.residencePermitPic2 &&
        params.residencePermitPic2.replace(imgHttp, imgDbPath)) ||
      "",
    photoUrl:
      (params.photoUrl && params.photoUrl.replace(imgHttp, imgDbPath)) || "",
    photoUrl2:
      (params.photoUrl2 && params.photoUrl2.replace(imgHttp, imgDbPath)) || ""
  };
...
exports.addPetRegAllInfo = async options => {
  const { openid, petRegId, uuid, params, orderNum } = options;
  ...
  const petRegInfoPromise = this.addPetregister(
    openid,
    petRegId,
    uuid,
    params,
...
exports.queryPayMentRecord = async petRegId => {
  const sql =
    " select * from pet_payment_record where pay_status = 0 and pet_id = ? ";
  return await conn.query(sql, [petRegId]);
};

// exports.isIdNumlogout = async idNumber => {
//   const sql =
//     " select m.id masterId,p.id petId,from pet_register_info p,pet_master m where m.id = p.master_id and p.pet_state in(2, 4, 5) and m.id_number = ? ";
//   const result = await conn.query(sql, [idNumber]);
//   if (!(result && result.length > 0)) {
//     return [];
//   }
//   const [masterId, petId] = result;
//   const deleteRegInfoPromise = conn.query(
//     `delete from pet_register_info where id ='${petId}' `
//   );
//   const deletePetMasterPromise = conn.query(
//     `delete from pet_master  where id ='${masterId}' `
//   );
//   const deletePetPrevPromise = conn.query(
//     `delete from pet_prevention_img  where pet_reg_id ='${petId}' `
//   );
//   const deleteWxRegInfoPromise = conn.query(
//     `delete from wx_pet_register_info where id ='${petId}' `
//   );
//   const deleteWxPetMasterPromise = conn.query(
//     `delete from wx_pet_master  where id ='${masterId}' `
//   );
//   const deleteWxPetPrevPromise = conn.query(
//     `delete from wx_pet_prevention_img  where pet_reg_id ='${petId}' `
//   );
//   return await Promise.all([
//     deleteRegInfoPromise,
//     deletePetMasterPromise,
//     deletePetPrevPromise,
//     deleteWxRegInfoPromise,
//     deleteWxPetMasterPromise,
